...
public class Directives {
    ...
    Directives(DataFetchingEnvironment env, ExecutionStepInfo step) {
        ...
        // Field directives
        fields.forEach(field -> directives.merge(Introspection.DirectiveLocation.FIELD,
                parseDirectives(field.getDirectives(), env), (directiveMap1, directiveMap2) -> {
                    directiveMap2
                            .forEach((directiveName, directiveValues) -> directiveMap1.merge(directiveName, directiveValues,
                                    (valueList1, valueList2) -> Stream.concat(valueList1.stream(), valueList2.stream())
                                            .collect(Collectors.toList())));
                    return directiveMap1;
                }));

        // Operation directives
        Map<String, List<Map<String, Object>>> operationDirectives = parseDirectives(
                env.getExecutionContext().getOperationDefinition().getDirectives(), env);
        ...
        if (step.hasParent() && step.getParent().getField() != null) {
            ...
            directives.put(Introspection.DirectiveLocation.INLINE_FRAGMENT,
                    parseDirectives(fragmentDirectiveCollector.getInlineFragmentDirs(), env));
            directives.put(Introspection.DirectiveLocation.FRAGMENT_SPREAD,
                    parseDirectives(fragmentDirectiveCollector.getFragmentDirs(), env));
            directives.put(Introspection.DirectiveLocation.FRAGMENT_DEFINITION,
                    parseDirectives(fragmentDirectiveCollector.getFragmentDefDirs(), env));
        } else {
    ...
    private Map<String, List<Map<String, Object>>> parseDirectives(List<Directive> directives, DataFetchingEnvironment env) {
        return directives.stream().collect(
                Collectors.groupingBy(Directive::getName,
                        Collectors.mapping(dir -> parseDirective(dir, env), Collectors.toList())));
    }
    ...
    private static class FragmentDirectiveCollector extends QueryVisitorStub {
        ...
        public void visitInlineFragment(QueryVisitorInlineFragmentEnvironment env) {
            ...
            boolean containsField = fieldsToFind.stream()
                    .anyMatch(field -> fragment.getSelectionSet().getSelections().contains(field));
            boolean isRelevant = containsField
                    || relevantFragments.stream().anyMatch(frag -> fragment.getSelectionSet().getSelections().contains(frag));
            if (isRelevant) {
        ...
        public void visitFragmentSpread(QueryVisitorFragmentSpreadEnvironment env) {
            ...
            boolean isRelevant = containsField || relevantFragments.stream()
                    .anyMatch(frag -> env.getFragmentDefinition().getSelectionSet().getSelections().contains(frag));
            if (isRelevant) {
