...
public class AnnotatedResolverBuilder extends FilteredResolverBuilder {
    ...
    public Collection<Resolver> buildQueryResolvers(ResolverBuilderParams params) {
        ...
        Stream<Resolver> methodInvokers = buildResolvers(params, Query.class, operationNameGenerator::generateQueryName, true)
                .stream();

        AnnotatedType beanType = params.getBeanType();
        Stream<Resolver> fieldAccessors = ClassUtils.getAnnotatedFields(ClassUtils.getRawType(beanType.getType()), Query.class)
                .stream()
                ...
                .map(field -> new Resolver(
                        operationNameGenerator.generateQueryName(
                                new OperationNameGeneratorParams<>(field, beanType, params.getQuerySourceBean(),
                                        messageBundle)),
                        ...
                        field.isAnnotationPresent(GraphQLComplexity.class)
                                ? field.getAnnotation(GraphQLComplexity.class).value()
                                : null));
    ...
    private Collection<Resolver> buildResolvers(ResolverBuilderParams params, Class<? extends Annotation> annotation,
        ...
        return ClassUtils.getAnnotatedMethods(ClassUtils.getRawType(beanType.getType()), annotation).stream()
                ...
                .map(method -> new Resolver(
                        nameGenerator
                                .apply(new OperationNameGeneratorParams<>(method, beanType, querySourceBean, messageBundle)),
                        ...
                        querySourceBean == null ? new MethodInvoker(method, beanType)
                                : new SingletonMethodInvoker(querySourceBean, method, beanType),
                        getReturnType(method, params),
                        argumentBuilder.buildResolverArguments(new ArgumentBuilderParams(method, beanType, inclusionStrategy,
                                params.getTypeTransformer(), params.getEnvironment())),
                        method.isAnnotationPresent(GraphQLComplexity.class)
                                ? method.getAnnotation(GraphQLComplexity.class).value()
                                : null))
