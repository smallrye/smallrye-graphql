...
public class ConvertingDeserializer extends JsonDeserializer implements ContextualDeserializer {
    ...
    private ConvertingDeserializer(AnnotatedType detectedType, InputConverter inputConverter, GlobalEnvironment environment,
    ...
    public JsonDeserializer<?> createContextual(DeserializationContext deserializationContext, BeanProperty beanProperty) {
        JavaType javaType = deserializationContext.getContextualType() != null ? deserializationContext.getContextualType()
                : extractType(beanProperty.getMember());
        ...
        if (inputConverter.supports(detectedType)) {
            return new ConvertingDeserializer(detectedType, inputConverter, environment,
                    (ObjectMapper) deserializationContext.getParser().getCodec());
        } else {
    ...
    public Object deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException {
        JavaType substituteType = deserializationContext.getTypeFactory()
                .constructType(environment.getMappableInputType(detectedType).getType());
