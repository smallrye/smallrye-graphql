...
public class PublicResolverBuilder extends FilteredResolverBuilder {
    ...
    public PublicResolverBuilder(String... basePackages) {
        ...
        this.deprecationReasonMapper = method -> javaDeprecationConfig.enabled && method.isAnnotationPresent(Deprecated.class)
    ...
    private Collection<Resolver> buildResolvers(ResolverBuilderParams params, Predicate<Method> filter,
        ...
        return Arrays.stream(rawType.getMethods())
                ...
                .map(method -> new Resolver(
                        nameGenerator.apply(new OperationNameGeneratorParams<>(method, beanType, querySourceBean,
                                params.getEnvironment().messageBundle)),
                        ...
                        argumentBuilder.buildResolverArguments(
                                new ArgumentBuilderParams(method, beanType, params.getInclusionStrategy(),
    ...
    protected boolean isPackageAcceptable(Method method, Class<?> beanType, String[] defaultPackages) {
        ...
        return method.getDeclaringClass().equals(beanType)
                || Arrays.stream(basePackages)
                        .anyMatch(basePackage -> ClassUtils.isSubPackage(method.getDeclaringClass().getPackage(), basePackage));
    }
}
