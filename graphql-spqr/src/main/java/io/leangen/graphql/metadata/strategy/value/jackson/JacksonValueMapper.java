...
public class JacksonValueMapper implements ValueMapper, InputFieldBuilder {
    ...
    private Stream<InputField> toInputField(PropertyDescriptor desc, BeanPropertyDefinition prop, ObjectMapper objectMapper,
            GlobalEnvironment environment) {
        if (!environment.inclusionStrategy.includeInputField(desc.declaringClass, desc.element, desc.type)) {
            return Stream.empty();
        }

        AnnotatedType deserializableType = resolveDeserializableType(desc.accessor, desc.type, desc.accessor.getType(),
                objectMapper);
        Object defaultValue = defaultValue(desc.declaringType, prop, desc.type, environment.typeTransformer, environment);
        return Stream.of(new InputField(prop.getName(), prop.getMetadata().getDescription(), desc.type, deserializableType,
                defaultValue, desc.element));
    }
    ...
    private AnnotatedType resolveDeserializableType(Annotated accessor, AnnotatedType realType, JavaType baseType,
        ...
        try {
            objectMapper.getDeserializationContext().getFactory().mapAbstractType(objectMapper.getDeserializationConfig(),
                    objectMapper.constructType(Map.class));
            JavaType refined = introspector.refineDeserializationType(objectMapper.getDeserializationConfig(), accessor,
                    baseType);
    ...
    protected Object defaultValue(AnnotatedType type, BeanPropertyDefinition prop, AnnotatedType fieldType,
            TypeTransformer typeTransformer, GlobalEnvironment environment) {
        ...
        return inputInfoGen
                .defaultValue(annotatedCandidates, fieldType, (element, t, val) -> fromString((String) val, t), environment)
                .orElse(null);
    }
    ...
    private static class PropertyDescriptor {
        ...
        PropertyDescriptor(AnnotatedType declaringType, Class<?> declaringClass, AnnotatedElement element, AnnotatedType type,
                Annotated accessor) {
