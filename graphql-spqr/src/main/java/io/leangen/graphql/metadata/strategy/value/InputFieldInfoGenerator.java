...
public class InputFieldInfoGenerator {
    ...
    public Optional<Object> defaultValue(List<AnnotatedElement> candidates, AnnotatedType type,
            DefaultValueProvider defaultValueProvider, GlobalEnvironment environment) {
        return candidates.stream()
                .filter(element -> element.isAnnotationPresent(DefaultValue.class))
                .findFirst()
                .map(element -> defaultValueProvider.getDefaultValue(element, type, environment.messageBundle
                        .interpolate(ReservedStrings.decode(element.getAnnotation(DefaultValue.class).value()))));
    }
}
