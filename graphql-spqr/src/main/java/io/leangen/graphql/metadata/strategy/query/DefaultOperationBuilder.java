...
public class DefaultOperationBuilder implements OperationBuilder {
    ...
    /**
     * @param typeInference Controls automatic type inference if multiple resolver methods for the same operation return
     *        different types,
    ...
    private Operation buildOperation(Type contextType, List<Resolver> resolvers, OperationDefinition.Operation operationType,
    ...
    protected AnnotatedType resolveJavaType(String operationName, List<Resolver> resolvers, MessageBundle messageBundle) {
        ...
        if (resolvers.stream()
                .anyMatch(resolver -> ClassUtils.containsTypeAnnotation(resolver.getReturnType(), GraphQLUnion.class))) {
            return unionize(returnTypes.toArray(new AnnotatedType[0]), messageBundle);
        }

        return resolveJavaType(returnTypes,
                "Multiple methods detected for operation \"" + operationName + "\" with different return types.");
    }
    ...
    protected List<OperationArgument> collectArguments(String operationName, List<Resolver> resolvers) {
        ...
        String errorPrefixTemplate = "Argument %s of operation \"" + operationName
                + "\" has different types in different resolver methods.";
        return argumentsByName.keySet().stream()
                .map(argName -> new OperationArgument(
                        resolveJavaType(argumentsByName.get(argName).stream().map(OperationArgument::getJavaType)
                                .collect(Collectors.toList()), String.format(errorPrefixTemplate, argName)),
                        argName,
                        argumentsByName.get(argName).stream().map(OperationArgument::getDescription).filter(Objects::nonNull)
                                .findFirst().orElse(""),
                        //						argumentsByName.get(argName).size() == resolvers.size() || argumentsByName.get(argName).stream().anyMatch(OperationArgument::isRequired),
                        argumentsByName.get(argName).stream().map(OperationArgument::getDefaultValue).filter(Objects::nonNull)
                                .findFirst().orElse(null),
                        argumentsByName.get(argName).stream().map(OperationArgument::getParameter).filter(Objects::nonNull)
                                .collect(Collectors.toList()),
    ...
    private AnnotatedType resolveJavaType(List<AnnotatedType> types, String errorPrefix) {
        errorPrefix = errorPrefix + " Types found: "
                + Arrays.toString(types.stream().map(type -> type.getType().getTypeName()).toArray()) + ". ";
        if (!typeInference.inferTypes
                && !types.stream().map(AnnotatedType::getType).allMatch(type -> type.equals(types.get(0).getType()))) {
            throw new TypeMappingException(
                    errorPrefix + "If this is intentional, and you wish GraphQL SPQR to infer the most " +
                            "common super type automatically, see " + Urls.Errors.CONFLICTING_RESOLVER_TYPES);
        }
        try {
            return ClassUtils.getCommonSuperType(types,
                    typeInference.allowObject ? GenericTypeReflector.annotate(Object.class) : null);
    ...
    /**
     * <p>
     * {@code NONE} - No type inference. Results in a {@link TypeMappingException} if multiple different types are encountered.
     * </p>
     * <p>
     * {@code LIMITED} - Automatically infer the common super type. Results in a {@link TypeMappingException} if no common
     * ancestors except
     * {@link Object}, {@link java.io.Serializable}, {@link Cloneable}, {@link Comparable} or
     * {@link java.lang.annotation.Annotation} are found.
     * </p>
     * <p>
     * {@code UNRESTRICTED} - Automatically infer the common super type. Results in {@link Object} if no common ancestors are
     * found.
     * </p>
     */
