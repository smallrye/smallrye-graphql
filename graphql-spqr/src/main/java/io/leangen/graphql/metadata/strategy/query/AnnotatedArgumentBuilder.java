...
public class AnnotatedArgumentBuilder implements ResolverArgumentBuilder {
    ...
    public List<OperationArgument> buildResolverArguments(ArgumentBuilderParams params) {
        ...
        for (int i = 0; i < resolverMethod.getParameterCount(); i++) {
            ...
            operationArguments.add(
                    buildResolverArgument(parameter, parameterType, params.getInclusionStrategy(), params.getEnvironment()));
        }
    ...
    protected Object defaultValue(Parameter parameter, AnnotatedType parameterType, DefaultValueProvider defaultValueProvider,
            GlobalEnvironment environment) {

        DefaultValue meta = parameter.getAnnotation(DefaultValue.class);
        if (meta == null)
            return null;
        return defaultValueProvider.getDefaultValue(parameter, parameterType,
                ReservedStrings.decode(environment.messageBundle.interpolate(meta.value())));
    }
}
