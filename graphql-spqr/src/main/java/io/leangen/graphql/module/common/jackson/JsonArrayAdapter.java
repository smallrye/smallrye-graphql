...
public class JsonArrayAdapter extends AbstractTypeSubstitutingMapper<List<JsonNode>>
        implements InputConverter<ArrayNode, List<JsonNode>>, OutputConverter<ArrayNode, List> {

    private static final AnnotatedType JSON = GenericTypeReflector.annotate(JsonNode.class);

    @Override
    public ArrayNode convertInput(List<JsonNode> substitute, AnnotatedType type, GlobalEnvironment environment,
    ...
    public AnnotatedType getSubstituteType(AnnotatedType original) {
        return GenericTypeReflector.annotate(new TypeToken<List<JsonNode>>() {
        }.getType(), original.getAnnotations());
    }
