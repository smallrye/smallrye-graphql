...
public class Scalars {
    ...
    public static final GraphQLScalarType GraphQLBase64String = new GraphQLScalarType("Base64String", "Base64-encoded binary",
            new Coercing() {
                @Override
                public Object serialize(Object dataFetcherResult) {
                ...
                public Object parseValue(Object input) {
                    if (input instanceof String) {
                        try {
                            ...
                            throw new CoercingParseValueException("Input string \"" + input + "\" is not a valid Base64 value",
                                    e);
                        }
                    }
                ...
                public Object parseLiteral(Object input) {
                    ...
                    try {
                        ...
                        throw new CoercingParseLiteralException("Input string \"" + input + "\" is not a valid Base64 value",
                                e);
                    }
                }
            });

    public static final GraphQLScalarType GraphQLClass = new GraphQLScalarType("Class", "A fully qualified class name",
            new Coercing() {
                @Override
                public Object serialize(Object dataFetcherResult) {
    ...
    public static final GraphQLScalarType GraphQLSqlDate = temporalScalar(java.sql.Date.class, "SqlDate",
            "a SQL compliant local date",
            s -> java.sql.Date.valueOf(LocalDate.parse(s)), i -> java.sql.Date.valueOf(i.atZone(ZoneOffset.UTC).toLocalDate()),
    ...
    public static final GraphQLScalarType GraphQLSqlTime = temporalScalar(Time.class, "SqlTime", "a SQL compliant local time",
            s -> Time.valueOf(LocalTime.parse(s)), i -> Time.valueOf(i.atZone(ZoneOffset.UTC).toLocalTime()),
            t -> t.toLocalTime().toString());

    public static final GraphQLScalarType GraphQLSqlTimestamp = temporalScalar(Timestamp.class, "SqlTimestamp",
            "a SQL compliant local date-time",
            s -> Timestamp.valueOf(LocalDateTime.parse(s)), i -> Timestamp.valueOf(i.atZone(ZoneOffset.UTC).toLocalDateTime()),
            t -> t.toLocalDateTime().toString());

    public static final GraphQLScalarType GraphQLCalendar = temporalScalar(Calendar.class, "Calendar",
            "a date-time with a time-zone",
            s -> GregorianCalendar.from(ZonedDateTime.parse(s)), i -> GregorianCalendar.from(i.atZone(ZoneOffset.UTC)),
            c -> c.toInstant().toString());
    ...
    public static final GraphQLScalarType GraphQLLocalDateTime = temporalScalar(LocalDateTime.class, "LocalDateTime",
            "a local date-time",
            LocalDateTime::parse, i -> i.atZone(ZoneOffset.UTC).toLocalDateTime());

    public static final GraphQLScalarType GraphQLZonedDateTime = temporalScalar(ZonedDateTime.class, "ZonedDateTime",
            "a date-time with a time-zone",
            ZonedDateTime::parse, i -> i.atZone(ZoneOffset.UTC));

    public static final GraphQLScalarType GraphQLOffsetDateTime = temporalScalar(OffsetDateTime.class, "OffsetDateTime",
            "a date-time with a UTC offset",
            OffsetDateTime::parse, i -> i.atOffset(ZoneOffset.UTC));

    public static final GraphQLScalarType GraphQLDurationScalar = temporalScalar(Duration.class, "Duration",
            "an amount of time",
    ...
    private static Object parseObjectValue(Value value, Map<String, Object> variables) {
        ...
        if (value instanceof ObjectValue) {
            ...
            ((ObjectValue) value).getObjectFields()
                    .forEach(field -> map.put(field.getName(), parseObjectValue(field.getValue(), variables)));
            return map;
        }
        //Should never happen
    ...
    public static <T> GraphQLScalarType temporalScalar(Class<?> type, String name, String description,
            ThrowingFunction<String, T> fromString, ThrowingFunction<Instant, T> fromDate) {
        return temporalScalar(type, name, description, fromString, fromDate, Object::toString);
    }

    public static <T> GraphQLScalarType temporalScalar(Class<?> type, String name, String description,
            ThrowingFunction<String, T> fromString, ThrowingFunction<Instant, T> fromDate,
            ThrowingFunction<T, String> toString) {
