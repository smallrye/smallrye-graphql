...
public class UnionTypeMapper extends UnionMapper {
    ...
    public GraphQLOutputType toGraphQLType(AnnotatedType javaType, OperationMapper operationMapper,
        ...
        return toGraphQLUnion(annotation.name(), annotation.description(), javaType, possibleJavaTypes, operationMapper,
                buildContext);
    }
    ...
    private List<AnnotatedType> getPossibleJavaTypes(AnnotatedType javaType, BuildContext buildContext) {
        ...
        if (possibleTypes.isEmpty() && annotation.possibleTypeAutoDiscovery()) {
            possibleTypes = buildContext.implDiscoveryStrategy.findImplementations(javaType, annotation.scanPackages(),
                    buildContext);
        }
        if (possibleTypes.isEmpty()) {
