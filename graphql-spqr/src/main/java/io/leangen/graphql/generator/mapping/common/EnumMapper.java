...
public class EnumMapper extends CachingMapper<GraphQLEnumType, GraphQLEnumType> {
    ...
    @Override
    public GraphQLEnumType toGraphQLType(String typeName, AnnotatedType javaType, OperationMapper operationMapper,
        ...
        buildContext.directiveBuilder.buildEnumTypeDirectives(javaType, buildContext.directiveBuilderParams())
                .forEach(directive -> enumBuilder.withDirective(operationMapper.toGraphQLDirective(directive, buildContext)));
        addOptions(enumBuilder, javaType, operationMapper, buildContext);
        return enumBuilder.build();
    }

    @Override
    public GraphQLEnumType toGraphQLInputType(String typeName, AnnotatedType javaType, OperationMapper operationMapper,
            BuildContext buildContext) {
        return toGraphQLType(typeName, javaType, operationMapper, buildContext);
    }

    private void addOptions(GraphQLEnumType.Builder enumBuilder, AnnotatedType javaType, OperationMapper operationMapper,
            BuildContext buildContext) {
        MessageBundle messageBundle = buildContext.messageBundle;
        sortEnumValues((Enum[]) ClassUtils.getRawType(javaType.getType()).getEnumConstants(),
                buildContext.typeInfoGenerator.getFieldOrder(javaType, messageBundle), messageBundle).stream()
    ...
    protected String getValueName(Enum<?> value, MessageBundle messageBundle) {
        ...
        return annotation != null && !annotation.value().isEmpty() ? messageBundle.interpolate(annotation.value())
                : value.name();
    }
    ...
    protected String getValueDeprecationReason(Enum<?> value, MessageBundle messageBundle) {
        io.leangen.graphql.annotations.Deprecated annotation = ClassUtils.getEnumConstantField(value)
                .getAnnotation(io.leangen.graphql.annotations.Deprecated.class);
        if (annotation != null) {
