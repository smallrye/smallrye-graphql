...
public class UnionInlineMapper extends UnionMapper {
    ...
    public GraphQLOutputType toGraphQLType(AnnotatedType javaType, OperationMapper operationMapper,
        ...
        List<AnnotatedType> possibleJavaTypes = Arrays
                .asList(((AnnotatedParameterizedType) javaType).getAnnotatedActualTypeArguments());
        return toGraphQLUnion(buildContext.interpolate(annotation.name()), buildContext.interpolate(annotation.description()),
                javaType, possibleJavaTypes, operationMapper, buildContext);
    }
