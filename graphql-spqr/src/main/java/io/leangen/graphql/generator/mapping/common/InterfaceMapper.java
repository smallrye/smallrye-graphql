...
public class InterfaceMapper extends CachingMapper<GraphQLInterfaceType, GraphQLInputObjectType> {
    ...
    @Override
    public GraphQLInterfaceType toGraphQLType(String typeName, AnnotatedType javaType, OperationMapper operationMapper,
        ...
        buildContext.directiveBuilder.buildInterfaceTypeDirectives(javaType, buildContext.directiveBuilderParams())
        ...
        if (isImplementationAutoDiscoveryEnabled(javaType)) {
            buildContext.implDiscoveryStrategy.findImplementations(javaType, getScanPackages(javaType), buildContext)
                    .forEach(impl -> getImplementingType(impl, operationMapper, buildContext)
                            .ifPresent(implType -> buildContext.typeRegistry.registerDiscoveredCovariantType(type.getName(),
                                    impl, implType)));
        }
    }
    ...
    protected boolean isImplementationAutoDiscoveryEnabled(AnnotatedType javaType) {
        return javaType.isAnnotationPresent(Interface.class)
                && javaType.getAnnotation(Interface.class).implementationAutoDiscovery();
    }

    @SuppressWarnings("WeakerAccess")
    protected String[] getScanPackages(AnnotatedType javaType) {
        return javaType.isAnnotationPresent(Interface.class) ? javaType.getAnnotation(Interface.class).scanPackages()
                : Utils.emptyArray();
    }

    private Optional<GraphQLObjectType> getImplementingType(AnnotatedType implType, OperationMapper operationMapper,
            BuildContext buildContext) {
