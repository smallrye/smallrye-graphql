...
public class TypeRegistryTest {
    ...
    public void knownTypesTest() {
        ...
        schema.getTypeMap().entrySet().stream()
                .filter(entry -> !GraphQLUtils.isIntrospectionType(entry.getValue())
                        && entry.getValue() != schema.getQueryType())
                .forEach(entry -> {
    ...
    public void nameUniquenessTest() {
        try (TestLog log = new TestLog(OperationMapper.class)) {
            try {
                ...
                /* Ignore exceptions from graphql-java, only testing validation in SPQR */}
    ...
    public static class Service {
        ...
        @Query
        public @GraphQLUnion(name = "mix") Union2<Street, Education> mix(@Info ResolutionEnvironment env,
                @GraphQLId(relayId = true) int id) {
    ...
    private static class NameCollision {
        @Query
        public FooInput test(Foo foo, FooInput fooInput) {
            return fooInput;
        }
    }
}
