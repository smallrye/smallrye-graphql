...
public class ResolverBuilderTest {
    ...
    public void bridgeMethodTest() {
        Collection<Resolver> resolvers = new PublicResolverBuilder(BASE_PACKAGES).buildQueryResolvers(new ResolverBuilderParams(
                new BaseServiceImpl<Number, String>(), new TypeToken<BaseServiceImpl<Number, String>>() {
                }.getAnnotatedType(),
    ...
    public void explicitIgnoreTest() {
        for (Collection<Resolver> resolvers : resolvers(new IgnoredMethods(), new BeanResolverBuilder(BASE_PACKAGES),
    ...
    public void fieldIgnoreTest() {
        for (Collection<Resolver> resolvers : resolvers(new IgnoredFields(), new BeanResolverBuilder(BASE_PACKAGES),
                new AnnotatedResolverBuilder())) {
            assertEquals(1, resolvers.size());
            assertEquals("notIgnored", resolvers.iterator().next().getOperationName());
        }
    ...
    public void impreciseBeanTypeTest() {
        GraphQLSchema schema = new TestSchemaGenerator()
                .withResolverBuilders(
                        new PublicResolverBuilder(BASE_PACKAGES).withOperationNameGenerator(new OperationNameGenerator() {
                            @Override
                ...
                .withOperationsFromSingleton(new One(), new TypeToken<BaseService<Person, Long>>() {
                }.getAnnotatedType())
                .withOperationsFromSingleton(new Two(), new TypeToken<BaseService<Number, Long>>() {
                }.getAnnotatedType())
                .generate();
