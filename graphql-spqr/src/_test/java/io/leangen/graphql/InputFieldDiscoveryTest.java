...
public class InputFieldDiscoveryTest {
    ...
    private static final io.leangen.graphql.metadata.InputField[] expectedFilteredDefaultFields = new io.leangen.graphql.metadata.InputField[] {
            expectedDefaultFields[0], expectedDefaultFields[2] };
    ...
    private Set<io.leangen.graphql.metadata.InputField> assertFieldNamesEqual(InputFieldBuilder mapper, Class typeToScan,
            io.leangen.graphql.metadata.InputField[] templates) {
        ...
        for (io.leangen.graphql.metadata.InputField template : templates) {
            Optional<io.leangen.graphql.metadata.InputField> field = fields.stream()
                    .filter(input -> input.getName().equals(template.getName())).findFirst();
    ...
    private void assertAllFieldsEqual(Set<io.leangen.graphql.metadata.InputField> fields1,
            Set<io.leangen.graphql.metadata.InputField> fields2) {
