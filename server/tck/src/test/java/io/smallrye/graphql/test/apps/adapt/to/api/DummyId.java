...
public final class DummyId implements Comparable<DummyId>, Serializable {
     ...
     * @since 4.1
     */
    public static DummyId getSmallestWithDate(final Date date) {
        return new DummyId(dateToTimestampSeconds(date), 0, (short) 0, 0, false);
    }
     ...
     * @param date the date
     */
    public DummyId(final Date date) {
     ...
     */
    public DummyId(final Date date, final int counter) {
     ...
     * @throws IllegalArgumentException if the high order byte of counter is not zero
     */
    public DummyId(final int timestamp, final int counter) {
        this(timestamp, counter, true);
    }

    private DummyId(final int timestamp, final int counter, final boolean checkCounter) {
        this(timestamp, RANDOM_VALUE1, RANDOM_VALUE2, counter, checkCounter);
    }

    private DummyId(final int timestamp, final int randomValue1, final short randomValue2, final int counter,
     ...
     */
    public DummyId(final String hexString) {
     ...
     */
    public DummyId(final byte[] bytes) {
     ...
     * @since 3.4
     */
    public DummyId(final ByteBuffer buffer) {
    ...
    @Override
    public int compareTo(final DummyId other) {
        if (other == null) {
    ...
    private static class SerializationProxy implements Serializable {
        ...
        SerializationProxy(final DummyId objectId) {
            bytes = objectId.toByteArray();
        }

        private Object readResolve() {
            return new DummyId(bytes);
        }
    }
