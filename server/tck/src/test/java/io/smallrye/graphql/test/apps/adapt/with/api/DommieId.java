...
public final class DommieId implements Comparable<DommieId>, Serializable {
     ...
     * <p>
     * The returned object id will compare as less than or equal to any other object id within the same second as the given
     * date, and
     ...
     * @since 4.1
     */
    public static DommieId getSmallestWithDate(final Date date) {
        return new DommieId(dateToTimestampSeconds(date), 0, (short) 0, 0, false);
    }
     ...
     * @param date the date
     */
    public DommieId(final Date date) {
     ...
     */
    public DommieId(final Date date, final int counter) {
     ...
     * @throws IllegalArgumentException if the high order byte of counter is not zero
     */
    public DommieId(final int timestamp, final int counter) {
        this(timestamp, counter, true);
    }

    private DommieId(final int timestamp, final int counter, final boolean checkCounter) {
        this(timestamp, RANDOM_VALUE1, RANDOM_VALUE2, counter, checkCounter);
    }

    private DommieId(final int timestamp, final int randomValue1, final short randomValue2, final int counter,
     ...
     */
    public DommieId(final String hexString) {
     ...
     */
    public DommieId(final byte[] bytes) {
     ...
     * @since 3.4
     */
    public DommieId(final ByteBuffer buffer) {
    ...
    @Override
    public int compareTo(final DommieId other) {
        if (other == null) {
    ...
    private static class SerializationProxy implements Serializable {
        ...
        SerializationProxy(final DommieId objectId) {
            bytes = objectId.toByteArray();
        }

        private Object readResolve() {
            return new DommieId(bytes);
        }
    }
